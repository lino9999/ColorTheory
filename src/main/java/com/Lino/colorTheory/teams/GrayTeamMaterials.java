package com.Lino.colorTheory.teams;

import org.bukkit.Material;
import java.util.HashSet;
import java.util.Set;

public class GrayTeamMaterials {
    public static Set<Material> getMaterials() {
        Set<Material> materials = new HashSet<>();

        materials.add(Material.GRAY_WOOL);
        materials.add(Material.GRAY_CONCRETE);
        materials.add(Material.GRAY_CONCRETE_POWDER);
        materials.add(Material.GRAY_TERRACOTTA);
        materials.add(Material.GRAY_GLAZED_TERRACOTTA);
        materials.add(Material.GRAY_STAINED_GLASS);
        materials.add(Material.GRAY_STAINED_GLASS_PANE);
        materials.add(Material.GRAY_CARPET);
        materials.add(Material.GRAY_BED);
        materials.add(Material.GRAY_BANNER);
        materials.add(Material.GRAY_SHULKER_BOX);
        materials.add(Material.GRAY_CANDLE);
        materials.add(Material.GRAY_DYE);

        materials.add(Material.LIGHT_GRAY_WOOL);
        materials.add(Material.LIGHT_GRAY_CONCRETE);
        materials.add(Material.LIGHT_GRAY_CONCRETE_POWDER);
        materials.add(Material.LIGHT_GRAY_TERRACOTTA);
        materials.add(Material.LIGHT_GRAY_GLAZED_TERRACOTTA);
        materials.add(Material.LIGHT_GRAY_STAINED_GLASS);
        materials.add(Material.LIGHT_GRAY_STAINED_GLASS_PANE);
        materials.add(Material.LIGHT_GRAY_CARPET);
        materials.add(Material.LIGHT_GRAY_BED);
        materials.add(Material.LIGHT_GRAY_BANNER);
        materials.add(Material.LIGHT_GRAY_SHULKER_BOX);
        materials.add(Material.LIGHT_GRAY_CANDLE);
        materials.add(Material.LIGHT_GRAY_DYE);

        materials.add(Material.STONE);
        materials.add(Material.COBBLESTONE);
        materials.add(Material.MOSSY_COBBLESTONE);
        materials.add(Material.STONE_BRICKS);
        materials.add(Material.MOSSY_STONE_BRICKS);
        materials.add(Material.CRACKED_STONE_BRICKS);
        materials.add(Material.CHISELED_STONE_BRICKS);
        materials.add(Material.SMOOTH_STONE);
        materials.add(Material.STONE_SLAB);
        materials.add(Material.SMOOTH_STONE_SLAB);
        materials.add(Material.STONE_STAIRS);
        materials.add(Material.COBBLESTONE_SLAB);
        materials.add(Material.COBBLESTONE_STAIRS);
        materials.add(Material.MOSSY_COBBLESTONE_SLAB);
        materials.add(Material.MOSSY_COBBLESTONE_STAIRS);
        materials.add(Material.STONE_BRICK_SLAB);
        materials.add(Material.STONE_BRICK_STAIRS);
        materials.add(Material.MOSSY_STONE_BRICK_SLAB);
        materials.add(Material.MOSSY_STONE_BRICK_STAIRS);
        materials.add(Material.COBBLESTONE_WALL);
        materials.add(Material.MOSSY_COBBLESTONE_WALL);
        materials.add(Material.STONE_BRICK_WALL);
        materials.add(Material.MOSSY_STONE_BRICK_WALL);
        materials.add(Material.STONE_BUTTON);
        materials.add(Material.STONE_PRESSURE_PLATE);

        materials.add(Material.ANDESITE);
        materials.add(Material.POLISHED_ANDESITE);
        materials.add(Material.ANDESITE_SLAB);
        materials.add(Material.POLISHED_ANDESITE_SLAB);
        materials.add(Material.ANDESITE_STAIRS);
        materials.add(Material.POLISHED_ANDESITE_STAIRS);
        materials.add(Material.ANDESITE_WALL);

        materials.add(Material.GRAVEL);
        materials.add(Material.FLINT);
        materials.add(Material.FLINT_AND_STEEL);

        materials.add(Material.IRON_ORE);
        materials.add(Material.DEEPSLATE_IRON_ORE);
        materials.add(Material.RAW_IRON);
        materials.add(Material.RAW_IRON_BLOCK);
        materials.add(Material.IRON_INGOT);
        materials.add(Material.IRON_BLOCK);
        materials.add(Material.IRON_NUGGET);
        materials.add(Material.IRON_BARS);
        materials.add(Material.IRON_DOOR);
        materials.add(Material.IRON_TRAPDOOR);
        materials.add(Material.HEAVY_WEIGHTED_PRESSURE_PLATE);
        materials.add(Material.CHAIN);

        materials.add(Material.IRON_HELMET);
        materials.add(Material.IRON_CHESTPLATE);
        materials.add(Material.IRON_LEGGINGS);
        materials.add(Material.IRON_BOOTS);
        materials.add(Material.IRON_SWORD);
        materials.add(Material.IRON_PICKAXE);
        materials.add(Material.IRON_AXE);
        materials.add(Material.IRON_SHOVEL);
        materials.add(Material.IRON_HOE);
        materials.add(Material.IRON_HORSE_ARMOR);

        materials.add(Material.CHAINMAIL_HELMET);
        materials.add(Material.CHAINMAIL_CHESTPLATE);
        materials.add(Material.CHAINMAIL_LEGGINGS);
        materials.add(Material.CHAINMAIL_BOOTS);

        materials.add(Material.DEEPSLATE);
        materials.add(Material.COBBLED_DEEPSLATE);
        materials.add(Material.POLISHED_DEEPSLATE);
        materials.add(Material.DEEPSLATE_TILES);
        materials.add(Material.DEEPSLATE_BRICKS);
        materials.add(Material.CHISELED_DEEPSLATE);
        materials.add(Material.CRACKED_DEEPSLATE_TILES);
        materials.add(Material.CRACKED_DEEPSLATE_BRICKS);
        materials.add(Material.COBBLED_DEEPSLATE_SLAB);
        materials.add(Material.POLISHED_DEEPSLATE_SLAB);
        materials.add(Material.DEEPSLATE_TILE_SLAB);
        materials.add(Material.DEEPSLATE_BRICK_SLAB);
        materials.add(Material.COBBLED_DEEPSLATE_STAIRS);
        materials.add(Material.POLISHED_DEEPSLATE_STAIRS);
        materials.add(Material.DEEPSLATE_TILE_STAIRS);
        materials.add(Material.DEEPSLATE_BRICK_STAIRS);
        materials.add(Material.COBBLED_DEEPSLATE_WALL);
        materials.add(Material.POLISHED_DEEPSLATE_WALL);
        materials.add(Material.DEEPSLATE_TILE_WALL);
        materials.add(Material.DEEPSLATE_BRICK_WALL);

        materials.add(Material.TUFF);
        materials.add(Material.TUFF_SLAB);
        materials.add(Material.TUFF_STAIRS);
        materials.add(Material.TUFF_WALL);
        materials.add(Material.CHISELED_TUFF);
        materials.add(Material.POLISHED_TUFF);
        materials.add(Material.POLISHED_TUFF_SLAB);
        materials.add(Material.POLISHED_TUFF_STAIRS);
        materials.add(Material.POLISHED_TUFF_WALL);
        materials.add(Material.TUFF_BRICKS);
        materials.add(Material.TUFF_BRICK_SLAB);
        materials.add(Material.TUFF_BRICK_STAIRS);
        materials.add(Material.TUFF_BRICK_WALL);
        materials.add(Material.CHISELED_TUFF_BRICKS);

        materials.add(Material.BASALT);
        materials.add(Material.POLISHED_BASALT);
        materials.add(Material.SMOOTH_BASALT);

        materials.add(Material.BLACKSTONE);
        materials.add(Material.POLISHED_BLACKSTONE);
        materials.add(Material.CHISELED_POLISHED_BLACKSTONE);
        materials.add(Material.POLISHED_BLACKSTONE_BRICKS);
        materials.add(Material.CRACKED_POLISHED_BLACKSTONE_BRICKS);
        materials.add(Material.GILDED_BLACKSTONE);
        materials.add(Material.BLACKSTONE_SLAB);
        materials.add(Material.POLISHED_BLACKSTONE_SLAB);
        materials.add(Material.POLISHED_BLACKSTONE_BRICK_SLAB);
        materials.add(Material.BLACKSTONE_STAIRS);
        materials.add(Material.POLISHED_BLACKSTONE_STAIRS);
        materials.add(Material.POLISHED_BLACKSTONE_BRICK_STAIRS);
        materials.add(Material.BLACKSTONE_WALL);
        materials.add(Material.POLISHED_BLACKSTONE_WALL);
        materials.add(Material.POLISHED_BLACKSTONE_BRICK_WALL);
        materials.add(Material.POLISHED_BLACKSTONE_BUTTON);
        materials.add(Material.POLISHED_BLACKSTONE_PRESSURE_PLATE);

        materials.add(Material.NETHERITE_INGOT);
        materials.add(Material.NETHERITE_BLOCK);
        materials.add(Material.ANCIENT_DEBRIS);
        materials.add(Material.NETHERITE_SCRAP);
        materials.add(Material.NETHERITE_HELMET);
        materials.add(Material.NETHERITE_CHESTPLATE);
        materials.add(Material.NETHERITE_LEGGINGS);
        materials.add(Material.NETHERITE_BOOTS);
        materials.add(Material.NETHERITE_SWORD);
        materials.add(Material.NETHERITE_PICKAXE);
        materials.add(Material.NETHERITE_AXE);
        materials.add(Material.NETHERITE_SHOVEL);
        materials.add(Material.NETHERITE_HOE);
        materials.add(Material.NETHERITE_UPGRADE_SMITHING_TEMPLATE);

        materials.add(Material.FURNACE);
        materials.add(Material.BLAST_FURNACE);
        materials.add(Material.SMOKER);
        materials.add(Material.DISPENSER);
        materials.add(Material.DROPPER);
        materials.add(Material.OBSERVER);
        materials.add(Material.PISTON);
        materials.add(Material.STICKY_PISTON);
        materials.add(Material.PISTON_HEAD);
        materials.add(Material.MOVING_PISTON);
        materials.add(Material.LEVER);
        materials.add(Material.TRIPWIRE_HOOK);

        materials.add(Material.CAULDRON);
        materials.add(Material.HOPPER);
        materials.add(Material.MINECART);
        materials.add(Material.CHEST_MINECART);
        materials.add(Material.FURNACE_MINECART);
        materials.add(Material.TNT_MINECART);
        materials.add(Material.HOPPER_MINECART);
        materials.add(Material.COMMAND_BLOCK_MINECART);
        materials.add(Material.RAIL);
        materials.add(Material.POWERED_RAIL);
        materials.add(Material.DETECTOR_RAIL);
        materials.add(Material.ACTIVATOR_RAIL);

        materials.add(Material.ANVIL);
        materials.add(Material.CHIPPED_ANVIL);
        materials.add(Material.DAMAGED_ANVIL);
        materials.add(Material.GRINDSTONE);
        materials.add(Material.STONECUTTER);
        materials.add(Material.LODESTONE);

        materials.add(Material.BUCKET);
        materials.add(Material.SHEARS);
        materials.add(Material.SHIELD);
        materials.add(Material.CROSSBOW);
        materials.add(Material.ARROW);
        materials.add(Material.SPECTRAL_ARROW);
        materials.add(Material.TIPPED_ARROW);
        materials.add(Material.BOW);
        materials.add(Material.FISHING_ROD);
        materials.add(Material.NAME_TAG);
        materials.add(Material.LEAD);
        materials.add(Material.COMPASS);
        materials.add(Material.RECOVERY_COMPASS);
        materials.add(Material.CLOCK);
        materials.add(Material.SPYGLASS);
        materials.add(Material.MAP);
        materials.add(Material.FILLED_MAP);

        materials.add(Material.COBWEB);
        materials.add(Material.DEAD_BUSH);
        materials.add(Material.WITHER_SKELETON_SKULL);
        materials.add(Material.WITHER_SKELETON_WALL_SKULL);
        materials.add(Material.SKELETON_SKULL);
        materials.add(Material.SKELETON_WALL_SKULL);
        materials.add(Material.SKELETON_SPAWN_EGG);
        materials.add(Material.STRAY_SPAWN_EGG);
        materials.add(Material.WITHER_SKELETON_SPAWN_EGG);
        materials.add(Material.SPIDER_SPAWN_EGG);
        materials.add(Material.CAVE_SPIDER_SPAWN_EGG);
        materials.add(Material.SILVERFISH_SPAWN_EGG);
        materials.add(Material.ENDERMITE_SPAWN_EGG);
        materials.add(Material.PHANTOM_SPAWN_EGG);
        materials.add(Material.RAVAGER_SPAWN_EGG);
        materials.add(Material.IRON_GOLEM_SPAWN_EGG);

        materials.add(Material.BLACK_WOOL);
        materials.add(Material.BLACK_CONCRETE);
        materials.add(Material.BLACK_CONCRETE_POWDER);
        materials.add(Material.BLACK_TERRACOTTA);
        materials.add(Material.BLACK_GLAZED_TERRACOTTA);
        materials.add(Material.BLACK_STAINED_GLASS);
        materials.add(Material.BLACK_STAINED_GLASS_PANE);
        materials.add(Material.BLACK_CARPET);
        materials.add(Material.BLACK_BED);
        materials.add(Material.BLACK_BANNER);
        materials.add(Material.BLACK_SHULKER_BOX);
        materials.add(Material.BLACK_CANDLE);
        materials.add(Material.BLACK_DYE);

        materials.add(Material.COAL);
        materials.add(Material.CHARCOAL);
        materials.add(Material.COAL_BLOCK);
        materials.add(Material.COAL_ORE);
        materials.add(Material.DEEPSLATE_COAL_ORE);
        materials.add(Material.GUNPOWDER);
        materials.add(Material.FIRE_CHARGE);
        materials.add(Material.ENDER_CHEST);
        materials.add(Material.RESPAWN_ANCHOR);
        materials.add(Material.LODESTONE);

        materials.add(Material.MUSIC_DISC_11);
        materials.add(Material.MUSIC_DISC_WAIT);
        materials.add(Material.MUSIC_DISC_OTHERSIDE);
        materials.add(Material.MUSIC_DISC_5);
        materials.add(Material.DISC_FRAGMENT_5);

        return materials;
    }
}
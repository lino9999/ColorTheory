package com.Lino.colorTheory.teams;

import org.bukkit.Material;
import java.util.HashSet;
import java.util.Set;

public class BlueTeamMaterials {
    public static Set<Material> getMaterials() {
        Set<Material> materials = new HashSet<>();

        materials.add(Material.BLUE_WOOL);
        materials.add(Material.BLUE_CONCRETE);
        materials.add(Material.BLUE_CONCRETE_POWDER);
        materials.add(Material.BLUE_TERRACOTTA);
        materials.add(Material.BLUE_GLAZED_TERRACOTTA);
        materials.add(Material.BLUE_STAINED_GLASS);
        materials.add(Material.BLUE_STAINED_GLASS_PANE);
        materials.add(Material.BLUE_CARPET);
        materials.add(Material.BLUE_BED);
        materials.add(Material.BLUE_BANNER);
        materials.add(Material.BLUE_SHULKER_BOX);
        materials.add(Material.BLUE_CANDLE);
        materials.add(Material.BLUE_DYE);

        materials.add(Material.LIGHT_BLUE_WOOL);
        materials.add(Material.LIGHT_BLUE_CONCRETE);
        materials.add(Material.LIGHT_BLUE_CONCRETE_POWDER);
        materials.add(Material.LIGHT_BLUE_TERRACOTTA);
        materials.add(Material.LIGHT_BLUE_GLAZED_TERRACOTTA);
        materials.add(Material.LIGHT_BLUE_STAINED_GLASS);
        materials.add(Material.LIGHT_BLUE_STAINED_GLASS_PANE);
        materials.add(Material.LIGHT_BLUE_CARPET);
        materials.add(Material.LIGHT_BLUE_BED);
        materials.add(Material.LIGHT_BLUE_BANNER);
        materials.add(Material.LIGHT_BLUE_SHULKER_BOX);
        materials.add(Material.LIGHT_BLUE_CANDLE);
        materials.add(Material.LIGHT_BLUE_DYE);

        materials.add(Material.CYAN_WOOL);
        materials.add(Material.CYAN_CONCRETE);
        materials.add(Material.CYAN_CONCRETE_POWDER);
        materials.add(Material.CYAN_TERRACOTTA);
        materials.add(Material.CYAN_GLAZED_TERRACOTTA);
        materials.add(Material.CYAN_STAINED_GLASS);
        materials.add(Material.CYAN_STAINED_GLASS_PANE);
        materials.add(Material.CYAN_CARPET);
        materials.add(Material.CYAN_BED);
        materials.add(Material.CYAN_BANNER);
        materials.add(Material.CYAN_SHULKER_BOX);
        materials.add(Material.CYAN_CANDLE);
        materials.add(Material.CYAN_DYE);

        materials.add(Material.LAPIS_LAZULI);
        materials.add(Material.LAPIS_BLOCK);
        materials.add(Material.LAPIS_ORE);
        materials.add(Material.DEEPSLATE_LAPIS_ORE);

        materials.add(Material.WATER);
        materials.add(Material.WATER_BUCKET);
        materials.add(Material.WATER_CAULDRON);
        materials.add(Material.ICE);
        materials.add(Material.PACKED_ICE);
        materials.add(Material.BLUE_ICE);
        materials.add(Material.FROSTED_ICE);
        materials.add(Material.SNOW);
        materials.add(Material.SNOW_BLOCK);
        materials.add(Material.SNOWBALL);
        materials.add(Material.POWDER_SNOW);
        materials.add(Material.POWDER_SNOW_BUCKET);
        materials.add(Material.POWDER_SNOW_CAULDRON);

        materials.add(Material.PRISMARINE);
        materials.add(Material.PRISMARINE_BRICKS);
        materials.add(Material.DARK_PRISMARINE);
        materials.add(Material.PRISMARINE_SLAB);
        materials.add(Material.PRISMARINE_BRICK_SLAB);
        materials.add(Material.DARK_PRISMARINE_SLAB);
        materials.add(Material.PRISMARINE_STAIRS);
        materials.add(Material.PRISMARINE_BRICK_STAIRS);
        materials.add(Material.DARK_PRISMARINE_STAIRS);
        materials.add(Material.PRISMARINE_WALL);
        materials.add(Material.PRISMARINE_SHARD);
        materials.add(Material.PRISMARINE_CRYSTALS);
        materials.add(Material.SEA_LANTERN);

        materials.add(Material.WARPED_STEM);
        materials.add(Material.STRIPPED_WARPED_STEM);
        materials.add(Material.WARPED_HYPHAE);
        materials.add(Material.STRIPPED_WARPED_HYPHAE);
        materials.add(Material.WARPED_PLANKS);
        materials.add(Material.WARPED_SLAB);
        materials.add(Material.WARPED_STAIRS);
        materials.add(Material.WARPED_FENCE);
        materials.add(Material.WARPED_FENCE_GATE);
        materials.add(Material.WARPED_DOOR);
        materials.add(Material.WARPED_TRAPDOOR);
        materials.add(Material.WARPED_SIGN);
        materials.add(Material.WARPED_HANGING_SIGN);
        materials.add(Material.WARPED_BUTTON);
        materials.add(Material.WARPED_PRESSURE_PLATE);
        materials.add(Material.WARPED_FUNGUS);
        materials.add(Material.WARPED_ROOTS);
        materials.add(Material.WARPED_NYLIUM);
        materials.add(Material.WARPED_WART_BLOCK);
        materials.add(Material.TWISTING_VINES);
        materials.add(Material.TWISTING_VINES_PLANT);

        materials.add(Material.DIAMOND);
        materials.add(Material.DIAMOND_BLOCK);
        materials.add(Material.DIAMOND_ORE);
        materials.add(Material.DEEPSLATE_DIAMOND_ORE);
        materials.add(Material.DIAMOND_HELMET);
        materials.add(Material.DIAMOND_CHESTPLATE);
        materials.add(Material.DIAMOND_LEGGINGS);
        materials.add(Material.DIAMOND_BOOTS);
        materials.add(Material.DIAMOND_SWORD);
        materials.add(Material.DIAMOND_PICKAXE);
        materials.add(Material.DIAMOND_AXE);
        materials.add(Material.DIAMOND_SHOVEL);
        materials.add(Material.DIAMOND_HOE);
        materials.add(Material.DIAMOND_HORSE_ARMOR);

        materials.add(Material.BLUE_ORCHID);
        materials.add(Material.CORNFLOWER);
        materials.add(Material.AZURE_BLUET);
        materials.add(Material.PITCHER_PLANT);
        materials.add(Material.PITCHER_POD);
        materials.add(Material.TORCHFLOWER);
        materials.add(Material.TORCHFLOWER_SEEDS);

        materials.add(Material.SOUL_TORCH);
        materials.add(Material.SOUL_WALL_TORCH);
        materials.add(Material.SOUL_LANTERN);
        materials.add(Material.SOUL_CAMPFIRE);
        materials.add(Material.SOUL_FIRE);
        materials.add(Material.SOUL_SAND);
        materials.add(Material.SOUL_SOIL);

        materials.add(Material.CONDUIT);
        materials.add(Material.HEART_OF_THE_SEA);
        materials.add(Material.NAUTILUS_SHELL);

        materials.add(Material.TROPICAL_FISH);
        materials.add(Material.PUFFERFISH);
        materials.add(Material.TROPICAL_FISH_BUCKET);
        materials.add(Material.PUFFERFISH_BUCKET);
        materials.add(Material.SALMON);
        materials.add(Material.COOKED_SALMON);
        materials.add(Material.SALMON_BUCKET);
        materials.add(Material.COD);
        materials.add(Material.COOKED_COD);
        materials.add(Material.COD_BUCKET);
        materials.add(Material.AXOLOTL_BUCKET);

        materials.add(Material.DOLPHIN_SPAWN_EGG);
        materials.add(Material.SQUID_SPAWN_EGG);
        materials.add(Material.GLOW_SQUID_SPAWN_EGG);
        materials.add(Material.GUARDIAN_SPAWN_EGG);
        materials.add(Material.ELDER_GUARDIAN_SPAWN_EGG);
        materials.add(Material.DROWNED_SPAWN_EGG);
        materials.add(Material.TURTLE_SPAWN_EGG);
        materials.add(Material.AXOLOTL_SPAWN_EGG);
        materials.add(Material.ALLAY_SPAWN_EGG);
        materials.add(Material.VEX_SPAWN_EGG);
        materials.add(Material.PHANTOM_SPAWN_EGG);

        materials.add(Material.GLOW_INK_SAC);
        materials.add(Material.INK_SAC);
        materials.add(Material.EXPERIENCE_BOTTLE);
        materials.add(Material.ENCHANTED_BOOK);
        materials.add(Material.WRITABLE_BOOK);
        materials.add(Material.WRITTEN_BOOK);
        materials.add(Material.KNOWLEDGE_BOOK);
        materials.add(Material.ENCHANTING_TABLE);

        materials.add(Material.TRIDENT);
        materials.add(Material.FISHING_ROD);
        materials.add(Material.CARROT_ON_A_STICK);
        materials.add(Material.WARPED_FUNGUS_ON_A_STICK);

        materials.add(Material.HEAVY_WEIGHTED_PRESSURE_PLATE);
        materials.add(Material.BUBBLE_COLUMN);
        materials.add(Material.BUBBLE_CORAL);
        materials.add(Material.BUBBLE_CORAL_BLOCK);
        materials.add(Material.BUBBLE_CORAL_FAN);
        materials.add(Material.BUBBLE_CORAL_WALL_FAN);
        materials.add(Material.DEAD_BUBBLE_CORAL);
        materials.add(Material.DEAD_BUBBLE_CORAL_BLOCK);
        materials.add(Material.DEAD_BUBBLE_CORAL_FAN);
        materials.add(Material.DEAD_BUBBLE_CORAL_WALL_FAN);

        materials.add(Material.TUBE_CORAL);
        materials.add(Material.TUBE_CORAL_BLOCK);
        materials.add(Material.TUBE_CORAL_FAN);
        materials.add(Material.TUBE_CORAL_WALL_FAN);
        materials.add(Material.DEAD_TUBE_CORAL);
        materials.add(Material.DEAD_TUBE_CORAL_BLOCK);
        materials.add(Material.DEAD_TUBE_CORAL_FAN);
        materials.add(Material.DEAD_TUBE_CORAL_WALL_FAN);

        materials.add(Material.KELP);
        materials.add(Material.KELP_PLANT);
        materials.add(Material.DRIED_KELP);
        materials.add(Material.DRIED_KELP_BLOCK);
        materials.add(Material.SEAGRASS);
        materials.add(Material.TALL_SEAGRASS);
        materials.add(Material.SEA_PICKLE);

        materials.add(Material.TURTLE_HELMET);
        materials.add(Material.TURTLE_EGG);
        materials.add(Material.PHANTOM_MEMBRANE);
        materials.add(Material.ELYTRA);
        materials.add(Material.TOTEM_OF_UNDYING);

        materials.add(Material.SHULKER_SHELL);
        materials.add(Material.SHULKER_BOX);
        materials.add(Material.SHULKER_SPAWN_EGG);

        materials.add(Material.CHORUS_FRUIT);
        materials.add(Material.POPPED_CHORUS_FRUIT);
        materials.add(Material.END_ROD);
        materials.add(Material.DRAGON_BREATH);
        materials.add(Material.LINGERING_POTION);
        materials.add(Material.POTION);
        materials.add(Material.SPLASH_POTION);
        materials.add(Material.TIPPED_ARROW);
        materials.add(Material.SPECTRAL_ARROW);

        materials.add(Material.ENDER_PEARL);
        materials.add(Material.ENDER_EYE);
        materials.add(Material.END_CRYSTAL);
        materials.add(Material.ENDER_CHEST);

        materials.add(Material.RESPAWN_ANCHOR);
        materials.add(Material.LODESTONE);
        materials.add(Material.COMPASS);
        materials.add(Material.RECOVERY_COMPASS);
        materials.add(Material.CLOCK);
        materials.add(Material.SPYGLASS);

        materials.add(Material.VERDANT_FROGLIGHT);
        materials.add(Material.PEARLESCENT_FROGLIGHT);
        materials.add(Material.BEACON);

        materials.add(Material.SCULK);
        materials.add(Material.SCULK_CATALYST);
        materials.add(Material.SCULK_SHRIEKER);
        materials.add(Material.SCULK_SENSOR);
        materials.add(Material.SCULK_VEIN);
        materials.add(Material.CALIBRATED_SCULK_SENSOR);
        materials.add(Material.ECHO_SHARD);

        materials.add(Material.MUSIC_DISC_11);
        materials.add(Material.MUSIC_DISC_13);
        materials.add(Material.MUSIC_DISC_CAT);
        materials.add(Material.MUSIC_DISC_CHIRP);
        materials.add(Material.MUSIC_DISC_FAR);
        materials.add(Material.MUSIC_DISC_MALL);
        materials.add(Material.MUSIC_DISC_MELLOHI);
        materials.add(Material.MUSIC_DISC_OTHERSIDE);
        materials.add(Material.MUSIC_DISC_PIGSTEP);
        materials.add(Material.MUSIC_DISC_RELIC);
        materials.add(Material.MUSIC_DISC_STAL);
        materials.add(Material.MUSIC_DISC_STRAD);
        materials.add(Material.MUSIC_DISC_WAIT);
        materials.add(Material.MUSIC_DISC_WARD);
        materials.add(Material.MUSIC_DISC_5);
        materials.add(Material.DISC_FRAGMENT_5);

        materials.add(Material.POTTED_BLUE_ORCHID);
        materials.add(Material.POTTED_CORNFLOWER);
        materials.add(Material.POTTED_AZURE_BLUET);
        materials.add(Material.POTTED_WARPED_FUNGUS);
        materials.add(Material.POTTED_WARPED_ROOTS);

        materials.add(Material.OXIDIZED_COPPER);
        materials.add(Material.OXIDIZED_CUT_COPPER);
        materials.add(Material.OXIDIZED_CUT_COPPER_SLAB);
        materials.add(Material.OXIDIZED_CUT_COPPER_STAIRS);
        materials.add(Material.OXIDIZED_CHISELED_COPPER);
        materials.add(Material.OXIDIZED_COPPER_DOOR);
        materials.add(Material.OXIDIZED_COPPER_TRAPDOOR);
        materials.add(Material.OXIDIZED_COPPER_GRATE);
        materials.add(Material.OXIDIZED_COPPER_BULB);
        materials.add(Material.WAXED_OXIDIZED_COPPER);
        materials.add(Material.WAXED_OXIDIZED_CUT_COPPER);
        materials.add(Material.WAXED_OXIDIZED_CUT_COPPER_SLAB);
        materials.add(Material.WAXED_OXIDIZED_CUT_COPPER_STAIRS);
        materials.add(Material.WAXED_OXIDIZED_CHISELED_COPPER);
        materials.add(Material.WAXED_OXIDIZED_COPPER_DOOR);
        materials.add(Material.WAXED_OXIDIZED_COPPER_TRAPDOOR);
        materials.add(Material.WAXED_OXIDIZED_COPPER_GRATE);
        materials.add(Material.WAXED_OXIDIZED_COPPER_BULB);

        return materials;
    }
}